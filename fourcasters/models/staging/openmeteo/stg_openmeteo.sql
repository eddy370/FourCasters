...
source_quotidienne_derniere_version as (

    select *
    from source_quotidienne

    qualify row_number() over (
        partition by
            lpad(trim(cast(code_INSEE as string)), 5, '0'),
...
quotidienne_cleaned as (
    ...
    select
        ...
        -- géographie enrichie depuis le référentiel
        g.Commune as ville,
    ...
    from source_quotidienne_derniere_version m
